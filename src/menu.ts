...
export function getSubMenuByKey(levels: number, params?: GetMenuParams, key?: string): IOEither<EnonicError, ReadonlyArray<MenuItem>> {
  return (key !== undefined)
    ? pipe(
        getContent({ key }),
        chain(getSubMenus(levels, params))
      )
    : right([]);
}
